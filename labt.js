            ...
            tooltip: {
                ...
                callbacks: {
                    label: function (context) {
                        ...
                        const avgReceived = trafficChart.data.datasets[2].data[context.index] || 0;
                        return `${label}: ${value} packets (Avg: ${avgReceived.toFixed(2)} packets)`;
                    }
                }
            },
            legend: {
...
const lowThreshold = 10; // Minimum packets for low traffic
const highThreshold = 40; // Maximum packets for high traffic

let totalReceivedPackets = 0;
let totalSentPackets = 0;
let packetCount = 0;

// Simulate real-time packet flow
function simulateNetworkTraffic() {
    const simulationInterval = setInterval(() => {
        const time = new Date().toLocaleTimeString();
        const receivedPackets = Math.floor(Math.random() * 50) + 1;
        const sentPackets = Math.floor(Math.random() * 30) + 1;
        ...
        trafficChart.data.datasets[0].data.push(receivedPackets);
        trafficChart.data.datasets[1].data.push(sentPackets);
        ...
        const avgReceived = totalReceivedPackets / packetCount;
        const avgSent = totalSentPackets / packetCount;

        trafficChart.data.datasets[2].data.push(avgReceived);

        // Change line color if traffic exceeds threshold
        if (receivedPackets < lowThreshold) {
            trafficChart.data.datasets[0].borderColor = 'rgba(0, 255, 0, 1)'; // Low traffic - green
        } else if (receivedPackets > highThreshold) {
            trafficChart.data.datasets[0].borderColor = 'rgba(255, 0, 0, 1)'; // High traffic - red
        } else {
            trafficChart.data.datasets[0].borderColor = 'rgba(75, 192, 192, 1)'; // Normal traffic - blue
        }
